...
void ask_secondaries_to_checkpoint()
{
    ...
    for(int i = 0; i < rowkey_range.size(); i++)
    {
        for(int j = 1; j < tablet_server_group[rowkey_range[i]].size(); j++)
        {
            ...
            write(socket_to_secondary, string(req_length).c_str(), string(req_length).length());      
            int return_val = write(socket_to_secondary, request_str.c_str(), strlen(request_str.c_str()));
